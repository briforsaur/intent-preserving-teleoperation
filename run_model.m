...
% Path of the setup script *relative to the model directory*
setup_script_path = "workspace_setup_scripts/teleop_sim_setup.m";
% It's easier to work in the model's directory temporarily
cd(model_dir)
update_model_workspace(model_name, setup_script_path);
wrkspcVars = get_model_workspace(model_name);
sim(model_name);
cd(start_dir)

% Saving all simulation inputs and outputs, except the directory strings
save(strcat(output_data_dir, datestr(now,30), "_", model_name),...
